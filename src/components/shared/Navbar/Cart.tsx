...
import { TOpenProps, TProduct } from "../../../types";
...
export default function Cart({
  ...
  const prevCartLength = useRef(products.length);

  useEffect(() => {
    if (products.length > prevCartLength.current) {
      setOpenCart(true);
    }
    prevCartLength.current = products.length;
  }, [products, products.length, setOpenCart]);
  ...
  return (
    <Dialog open={openCart} onClose={setOpenCart} className="relative z-[250]">
      <DialogBackdrop
        transition
